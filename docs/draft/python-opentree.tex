...
There are several tools to generate phylogenies from DNA sequence data from genetic
databases such as GenBank, e.g., \citep{smith2009mega, sanderson2008phylota, antonelli2014supersmart}.
To generate good quality phylogenies \textit{de novo} with these tools it is required
...
Resources such as TreeBase \citep{piel2000treebase} and STbase \citep{mcmahon2015stbase}
store published phylogenies and have applications to browse and download Phylogenies
from a group of interest in computer readable formats.
Phylofinder \citep{chen2008phylofinder} is a tool that browses phylogenies from
other databases by enabling taxonomic queries
to identify trees in a database containing the exact name of the query taxon and
synonomies, providing spelling suggestions for the query when there is no match.
These resources however provide phylogenies as is (often with incomplete
taxonomic sampling), with no possibility of obtaining
subset of taxa within phylogenies or combinations of pyhlogenies directly from the database,
requiring further processing from the users if their study requires incorporating
missing taxa, extracting a phylogeny of a subset of taxa or combining the trees to match tips in
a trait dataset for a comparative study, for example.
...
% from GenBank is not enough to get the most complete phylogeny of a group, when genetic

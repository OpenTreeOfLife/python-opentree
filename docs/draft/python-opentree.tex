...
Open Tree of Life provides web-service APIs to make the tree estimate, unified taxonomy, and input phylogenetic data available to anyone.
we describe the python package \texttt{opentree}, which provides a user friendly python wrapper for theses APIs and a set of scripts and tutorials for straightforward downstream data analysis functionality. 
...
\texttt{opentree} wraps the OpenTree taxonomy and TNRS APIs for ease of integrating taxonomy and TNRS queries with downstream analyses. In addition, \texttt{opentree} provides helpers for quickly searching the text download of the taxonomy,using the  which can be more efficient for some bulk queries.

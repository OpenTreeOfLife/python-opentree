...
class DendropyConvert(object):
    ...
    def tree_list_from_newicks(self, newick_list, suppress_internal_node_taxa=False):
        concat = '\n'.join(newick_list)
        return dendropy.TreeList.get(data=concat, schema="newick", suppress_internal_node_taxa=suppress_internal_node_taxa)
